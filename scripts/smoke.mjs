...
try {
  ...
  // --- 種目管理: 種目編集（名前・部位・対象筋）を開いて保存 ---
  await page.locator('button:has-text("種目管理")').click();
  await page.waitForSelector('text=カスタム種目を追加');
  // ベンチプレス行をタップして編集
  await page.locator('button:has-text("ベンチプレス")').first().click();
  await page.waitForSelector('text=種目を編集');
  await page.waitForTimeout(300);
  await shot('13-exercise-editor.png');
  // 「大胸筋下部」をタップして主働筋に追加 → 保存（手動カスタム化）
  await page.locator('div.fixed.z-50 button:has-text("大胸筋下部")').first().click();
  await page.locator('div.fixed.z-50 button:has-text("保存する")').click();
  await page.waitForSelector('text=カスタム');
